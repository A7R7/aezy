...
test('official Thread cancellation closes only its own active transport; disconnect closes all', async t => {
  let closed = 0
  const { gateway, request } = await fixture(t, (req, res) => {
    req.resume()
  ...
  const a = await request(), b = await request(input, { 'thread-id': 'thread-b' })
  assert.equal(gateway.status().active, 2)
  gateway.cancelThread('foreign')
  gateway.cancelThread(null)
  assert.equal(gateway.status().active, 2)
  gateway.cancelThread('thread-a')
  for (let i = 0; i < 100 && (closed !== 1 || gateway.status().active !== 1); i++) await pause(10)
  assert.equal(closed, 1)
  assert.equal(gateway.status().active, 1)
  gateway.cancelAll()
  for (let i = 0; i < 100 && (closed !== 2 || gateway.status().active); i++) await pause(10)
  assert.equal(closed, 2)
  assert.equal(gateway.status().active, 0)
  assert.equal(gateway.status().cancelled, 2)
  assert.equal(gateway.status().completed, 0)
