  ...
  return (
    <div className="usage-filters">
      <div className="usage-segmented" role="group" aria-label={t("logs.filter.surface.label")}>
        {(["all", ...surfaces.map(s => s.id)] as UsageSurface[]).map(choice => {
          ...
          return (
            ...
            >
              ...
              <span className="usage-source-label">
                {label}
              </span>
            </button>
          );
      ...
      {(
        <div className="usage-cost-row" role="note">
          ...
          <span className="stat-value mono usage-cost-value">
            {summary.estimatedCostUsd === undefined ? 'Unavailable — no matched price' : formatUsdEstimate(summary.estimatedCostUsd, locale)}
          </span>
    ...
    <>
      ...
      {state.showSkeleton && !data ? (
        ...
        <>
          ...
          {!!data?.persistenceErrors && <Notice tone="err">Some observations could not be persisted. Usage is incomplete.</Notice>}
