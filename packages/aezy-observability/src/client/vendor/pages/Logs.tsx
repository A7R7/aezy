...
import { useSurfaces, surfaceLabel } from "../aezy-api";
...
export interface LogEntry {
  unit?: 'request' | 'turn' | 'usage-notification';
  accounting?: boolean;
  requestId?: string;
  timestamp: number;
  model: string;
...
export default function Logs({ apiBase }: { apiBase: string }) {
  ...
  // Aezy query parameters are the source of truth (preserving DSH URL state),
  ...
  const loadLogs = useCallback(async (signal: AbortSignal): Promise<LogEntry[]> => {
    ...
    const body = await res.json() as LogEntry[] | { logs?: LogEntry[]; persistenceErrors?: number };
  ...
  return (
    <div className="logs-page">
      ...
      {storageError && <Notice tone="err">Some observations could not be persisted. This report may be incomplete; check the Aezy storage directory.</Notice>}
      {debugMounted && (
        <div
          role="tabpanel"
          id="logs-panel-debug"
      ...
      <div className="logs-toolbar">
        ...
        <div className="segmented logs-segmented" role="radiogroup" aria-label={t("logs.filter.surface.label")}>
          {(["all", ...surfaces.map(s => s.id)]).map(surface => (
            <button
              key={surface}
              type="button"
              role="radio"
        ...
        */}
        <label className="muted text-control logs-filter-field">
          ...
          />
          {t("logs.filter.interceptedHelpersOnly")} (not enabled in Aezy)
        </label>
        <label className="muted text-control logs-filter-field">
            ...
            <tbody>
              ...
              {virtualRows.map(virtualRow => {
               ...
               >
                 ...
                 <td className="mono log-col-model" title={modelTitle(log, t)}>
                  <span className="logs-model-cell">
                   <span>{modelLabel(log.resolvedModel ?? log.model)}</span>
                      ...
                      {log.surface && <span className="badge badge-accent">{surfaceLabel(log.surface, surfaces)}</span>}
                  ...
                  <td>
                    <span className="log-status-cell">
                      <span className="mono font-semibold" style={{ color: statusColor(log.status) }}>{log.unit === 'usage-notification' ? 'Reported' : log.status}</span>
                      <button
                        type="button"
                 ...
                 <td className="num log-col-duration">{log.unit === 'usage-notification' ? '—' : `${log.durationMs}ms`}</td>
                </tr>
                );
              })}
              {paddingBottom > 0 && (
      ...
      <div className="modal-card log-detail-card" onClick={event => event.stopPropagation()} role="document">
        <div className="modal-head">
          <h3 id="log-detail-title">
            <span className="mono" style={{ color: statusColor(detail.status) }}>{detail.unit === 'usage-notification' ? 'Usage reported' : detail.status}</span>
            {detailInfo && detail.unit !== 'usage-notification' && <span className="logs-detail-info">{detailInfo.label}</span>}
          </h3>
          <button type="button" className="btn btn-ghost btn-sm" onClick={onClose} aria-label={t("common.cancel")}><IconX /></button>
        </div>
        {detailInfo && detail.unit !== 'usage-notification' && <p className="modal-desc">{detailInfo.description}</p>}
        ...
        <p className="muted">Accounting unit: {detail.unit ?? "request"}. Official notifications have no HTTP status or transport timing. {detail.accounting === false ? 'Terminal diagnostic only; excluded from Usage to avoid duplicate accounting.' : ''}</p>
        ...
        <section className="log-detail-section" aria-labelledby="log-detail-performance">
          ...
          <div className="log-detail-grid">
            <span className="muted">{t("logs.col.duration")}</span><span className="mono">{detail.unit === 'usage-notification' ? 'Unavailable' : `${detail.durationMs}ms`}</span>
              ...
              <div className="log-detail-grid">
                ...
                {cost.estimate.price && (
                  <>
                    ...
                    <span className="muted">Price snapshot</span><span className="log-detail-break">{cost.estimate.price.verifiedAt ?? "Unspecified date"} · {cost.estimate.price.sourceRef ?? "User configuration"}</span>
                  </>
                )}
              </div>
