...
> **Archived：rc.7 自动生成的历史研究附录。** 不作为当前清单维护。rc.8 增量见
> [归档报告](dsh-rc8-impact.md)，当前基线变化见
> [rc.1 影响报告](../../reference/dsh-0.1.1-rc1-impact.md)。不要把“包”与“默认运行中的
> 插件实例”混为一谈。
...
| `@deepseek-ai/dsh-client-schema-form` | Web Client 基础库 | — | Schema/draft model layer for settings editors: rehydrates a serialized schemastery schema, validates drafts, and edits them immutably by path |
...
| `@deepseek-ai/dsh-client-web-react` | Web Client 基础库 | — | Shell-side React glue: createSlotRenderer, SessionProvider, bindSnapshotSelector (uSES bridge), useInvoke |
