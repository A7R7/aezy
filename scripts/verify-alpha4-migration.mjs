...
const baseUrl = process.env.AEZY_ALPHA_GATE_URL ?? 'http://127.0.0.1:3294'
const launchToken = process.env.AEZY_ALPHA_GATE_TOKEN
if (!launchToken) throw new Error('AEZY_ALPHA_GATE_TOKEN is required for the alpha.4 migration gate')
const restart = process.env.AEZY_ALPHA4_RESTART === '1'
const snapshotPath = process.env.AEZY_ALPHA4_SNAPSHOT ?? '/tmp/aezy-alpha4-migration-sessions.json'
...
assert.ok(setCookie, 'alpha.4 bootstrap returned no signed cookie')
...
async function rpcEnvelope(method, args) {
  ...
  const response = await fetch(`${baseUrl}/api/${method}`, {
    ...
    body: JSON.stringify({
      type: 'client-request',
      rpcId: `alpha4-migration-${String(rpcSequence)}`,
      method,
      payload: { args },
    }),
...
if (restart) {
  ...
  snapshot = {
    ...
    sessions: [
      { id: `aezy-alpha4-standard-${nonce}`, preset: 'standard' },
      { id: `aezy-alpha4-app-server-${nonce}`, preset: 'codex-app-server' },
      { id: `aezy-alpha4-codex-inspired-${nonce}`, preset: CODEX_INSPIRED_PRESET_ID },
    ],
  }
...
const missing = await rpcEnvelope('session/page', {
  request: {
    address: { kind: 'session', sessionId: 'aezy-alpha4-intentionally-missing' },
    maxMessages: 1,
  },
})
