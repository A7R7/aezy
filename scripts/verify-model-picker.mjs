...
try {
  ...
  await model.click()
  await page.getByRole('menuitem', { name: /GPT-5.6-Sol/ }).waitFor()
  ...
  await page.getByRole('menuitem', { name: /GPT-5.6-Sol/ }).click()
  await page.waitForFunction(() => document.querySelector('[aria-label="Model"]')?.textContent.includes('GPT-5.6-Sol'))
  await page.getByRole('button', { name: 'Reasoning effort', exact: true }).click()
  ...
  const receipt = { timestamp: new Date().toISOString(), profile: alphaProfileName,
    paidModelCalls: 0, nativeSelectCount: 0, standardAstra: true, astraEfforts: effortLabels,
