...
const baseUrl = process.env.AEZY_ALPHA_GATE_URL ?? 'http://127.0.0.1:3395'
const launchToken = process.env.AEZY_ALPHA_GATE_TOKEN
if (!launchToken) throw new Error('AEZY_ALPHA_GATE_TOKEN is required for the rc.1 migration gate')
const restart = process.env.AEZY_RC1_RESTART === '1'
const snapshotPath = process.env.AEZY_RC1_SNAPSHOT ?? '/tmp/aezy-rc1-migration-sessions.json'
const carryoverSessionId = process.env.AEZY_RC1_CARRYOVER_SESSION
...
assert.ok(setCookie, 'rc.1 bootstrap returned no signed cookie')
...
async function rpcEnvelope(method, args) {
  ...
  const response = await fetch(`${baseUrl}/api/${method}`, {
    ...
    body: JSON.stringify({
      type: 'client-request',
      rpcId: `rc1-migration-${String(rpcSequence)}`,
      method,
      payload: { args },
    }),
...
if (restart) {
  ...
  snapshot = {
    ...
    sessions: [
      { id: `aezy-rc1-standard-${nonce}`, preset: 'standard' },
      { id: `aezy-rc1-app-server-${nonce}`, preset: 'codex-app-server' },
      { id: `aezy-rc1-codex-inspired-${nonce}`, preset: CODEX_INSPIRED_PRESET_ID },
    ],
  }
...
if (carryoverSessionId) {
  const carryover = listed.items.find(item => item.sessionId === carryoverSessionId)
  assert.ok(carryover, `alpha.4 carryover Session ${carryoverSessionId} was not recovered`)
  assert.equal(Number.isSafeInteger(carryover.projections?.asOfSeq), true)
}
for (const session of snapshot.sessions) {
...
const missing = await rpcEnvelope('session/page', {
  request: {
    address: { kind: 'session', sessionId: 'aezy-rc1-intentionally-missing' },
    maxMessages: 1,
  },
})
...
process.stdout.write(`${JSON.stringify({
  ...
  carryoverSession: carryoverSessionId ?? null,
}, null, 2)}\n`)
