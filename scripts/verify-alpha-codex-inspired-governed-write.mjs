...
if (!launchToken) throw new Error('AEZY_ALPHA_GATE_TOKEN is required for the governed write gate')
if (process.env.AEZY_CODEX_INSPIRED_DOGFOOD !== '1') {
  throw new Error('AEZY_CODEX_INSPIRED_DOGFOOD=1 is required for the governed write gate')
}
...
const fixture = await mkdtemp('/tmp/aezy-codex-inspired-write-')
const sessionId = `aezy-codex-inspired-write-${Date.now().toString(36)}`
const filename = 'codex-inspired-governed-write.txt'
const content = 'CODEX_INSPIRED_GOVERNED_WRITE_OK\n'
...
async function rpc(method, args) {
  ...
  const response = await fetch(`${baseUrl}/api/${method}`, {
    ...
    body: JSON.stringify({
      type: 'client-request',
      rpcId: `codex-inspired-write-${String(rpcSequence)}`,
      method,
      payload: { args },
    }),
...
try {
  ...
  socket.on('message', data => {
    try {
      ...
      if (message.type !== 'item' || message.streamId !== 'codex-inspired-write-events') return
  ...
  socket.send(JSON.stringify({
    type: 'open',
    streamId: 'codex-inspired-write-events',
    endpoint: '$events',
    payload: { args: {} },
  }))
  await Promise.race([
  ...
  const explanation = await post('/aezy/api/security/explain', {
    cwd: fixture,
    tool: 'write',
    arguments: { file_path: filename, content },
  })
  ...
  await rpc('session/prompt', { request: {
    ...
    content: [{
      type: 'text',
      text: [
        `Use the structured write tool exactly once to create ${filename}.`,
        'Its exact content must be CODEX_INSPIRED_GOVERNED_WRITE_OK followed by one newline.',
        'Do not use shell, edit, or any other tool.',
        'After the governed write succeeds, reply with exactly CODEX_INSPIRED_GOVERNED_WRITE_OK.',
      ].join(' '),
    }],
  ...
  assert.ok(summary, 'codex-inspired governed write Session disappeared')
  assert.equal(summary.running, false, 'codex-inspired governed write Turn did not settle')
  ...
  const writeCall = events.find(event => event.type === 'tool/call' && event.data.name === 'write')
  const writeResult = events.find(event => event.type === 'tool/result'
    && event.data.message?.source?.callId === writeCall?.data.callId)
  ...
  assert.ok(writeCall, 'codex-inspired model did not issue a structured DSH write')
  assert.ok(writeResult, 'codex-inspired structured write produced no durable result')
  assert.match(visible, /CODEX_INSPIRED_GOVERNED_WRITE_OK/)
  ...
  assert.ok(turn, 'Journal did not publish the codex-inspired governed Turn')
  ...
  assert.ok(audit, 'codex-inspired governed write did not enter the Security audit')
